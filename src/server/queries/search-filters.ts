...
export function buildNonLocationFilterConditions(
  ...
  if (filters.minArea && filters.minArea > 0) {
    conds.push(
      sql`COALESCE(${properties.squareMeter}, ROUND(${properties.builtSurfaceArea})) >= ${filters.minArea}`,
    );
  }

  if (filters.maxArea && filters.maxArea > 0) {
    conds.push(
      sql`COALESCE(${properties.squareMeter}, ROUND(${properties.builtSurfaceArea})) <= ${filters.maxArea}`,
    );
  }
